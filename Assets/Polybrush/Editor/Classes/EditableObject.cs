    ...
    class EditableObject : IEquatable<EditableObject>, IValid
	{
        ...
        private void Initialize(GameObject go)
        {
            ...
            if (meshRenderer != null || _skinMeshRenderer != null)
            {
                ...
                mesh.name = GetMeshInstanceName(gameObjectAttached);
            }
        ...
        internal static string GetMeshInstanceName(GameObject gameObject)
        {
            // Store the full EntityId explicitly; its display format is not a serialization format.
            return k_MeshInstancePrefix + EntityId.ToULong(gameObject.GetEntityId())
                .ToString(System.Globalization.CultureInfo.InvariantCulture);
        }

        internal static EntityId GetMeshId(Mesh mesh)
        {
            if (mesh == null || !mesh.name.StartsWith(k_MeshInstancePrefix, StringComparison.Ordinal))
                return EntityId.None;

            ulong meshId;
            if (!ulong.TryParse(mesh.name.Substring(k_MeshInstancePrefix.Length),
                System.Globalization.NumberStyles.None,
                System.Globalization.CultureInfo.InvariantCulture, out meshId))
                return EntityId.None;

            return EntityId.FromULong(meshId);
        }
		...
		static bool MeshInstanceMatchesGameObject(Mesh mesh, GameObject go)
		{
            ...
            EntityId gameObjectId = go.GetEntityId();
			EntityId meshId = GetMeshId(mesh);

			// If the mesh id doesn't parse to an ID it's definitely not an instance
			if(meshId == EntityId.None)
				return false;
			...
			if (EditorUtility.EntityIdToObject(meshId) == null)
			{
				mesh.name = GetMeshInstanceName(go);
				return true;
			}
